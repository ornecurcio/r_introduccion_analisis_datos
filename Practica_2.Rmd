...
vector <- numeric(100) # Creamos un vector de 100 elementos, inicializado con ceros

for (i in 1:100) {
  vector[i] <- 1/i  # Asignamos a cada elemento el valor correspondiente de la serie
}
sum(vector)
...
```{r}
url="https://www.asexuality.org/en/topic/123256-asexuality-sexual-orientationlexicon-read-me/"
pagina = readLines(url) # ya tenemos toda la info de la pagina
typeof(pagina)
```


### 2. El comando $str(funcion$\_$a$\_$estudiar)$ nos dice los argumentos de la función. Escribir $str(mean)$. ¿Qué le parece que son los $\dots$? Si no entendió los $\dots$ no se preocupe, en el ejercicio 10 lo va a entender. Escriba $str(sum)$.
